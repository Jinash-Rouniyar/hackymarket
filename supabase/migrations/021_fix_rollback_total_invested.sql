-- ============================================
-- Migration 021: Fix rollback_trade total_invested for SELL
--
-- Bug: The SELL branch of rollback_trade did not restore
-- total_invested, but execute_sell (migration 019) does
-- decrease total_invested by the payout. This caused
-- N/A resolutions after rollbacks to under-refund users.
--
-- This migration:
-- 1. Fixes the rollback_trade function
-- 2. Credits affected users on N/A-resolved markets
-- 3. Corrects stale total_invested in positions
-- ============================================

-- 1. Fix rollback_trade: restore total_invested in SELL branch
...
BEGIN
  ...
  ELSIF v_trade.type = 'SELL' THEN
    ...
    -- Reverse position: restore shares and total_invested
    -- execute_sell (migration 019) does: total_invested -= payout, so we reverse it
    IF v_trade.outcome = 'YES' THEN
      UPDATE public.positions
      SET yes_shares = yes_shares + v_trade.shares,
          total_invested = total_invested + v_trade.amount
...
-- Permissions (unchanged from 020)
...
-- 2. Fix positions: restore missing total_invested for rolled-back SELLs
--    on N/A-resolved markets
UPDATE public.positions
SET total_invested = positions.total_invested + sf.missing
FROM (
  SELECT t.user_id, t.market_id, SUM(t.amount) AS missing
  FROM public.trades t
  JOIN public.markets m ON m.id = t.market_id
  WHERE t.type = 'SELL'
    AND t.is_rolled_back = true
    AND m.status = 'resolved'
    AND m.resolution = 'N/A'
  GROUP BY t.user_id, t.market_id
) sf
WHERE positions.user_id = sf.user_id
  AND positions.market_id = sf.market_id;

-- 3. Credit affected users the difference, accounting for GREATEST(0, ...) clamping
UPDATE public.profiles
SET balance = balance + sub.credit_owed
FROM (
  SELECT
    pos.user_id,
    SUM(
      GREATEST(0, pos.total_invested)
      - GREATEST(0, pos.total_invested - sf.missing)
    ) AS credit_owed
  ...
  JOIN (
    ...
    WHERE t.type = 'SELL'
      ...
      AND m.resolution = 'N/A'
    GROUP BY t.user_id, t.market_id
  ) sf ON sf.user_id = pos.user_id AND sf.market_id = pos.market_id
  GROUP BY pos.user_id
) sub
WHERE profiles.id = sub.user_id
  AND sub.credit_owed > 0;
