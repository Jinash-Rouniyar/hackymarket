-- ============================================
-- Migration 014: Reduce total_invested on sell
--
-- Previously, execute_sell never reduced total_invested,
-- which meant N/A resolution refunds could exceed what
-- the user actually had at stake. A user could pump
-- total_invested via repeated buy/sell cycles and then
-- profit from an N/A resolution.
--
-- Fix: subtract v_payout (leaves returned) from
-- total_invested when updating the position after a sell.
-- ============================================
...
BEGIN
  ...
  -- Update position: reduce shares AND total_invested by the payout (leaves returned)
  IF p_outcome = 'YES' THEN
    UPDATE public.positions
    SET yes_shares = yes_shares - p_shares,
        total_invested = GREATEST(0, total_invested - v_payout)
    WHERE user_id = p_user_id AND market_id = p_market_id;
  ELSE
    UPDATE public.positions
