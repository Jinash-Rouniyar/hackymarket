-- ============================================
-- Migration 019: Secure RPC functions against direct access
--
-- CRITICAL SECURITY FIX: The SECURITY DEFINER functions (execute_trade,
-- execute_sell, create_market, resolve_market, verify_qr_code) were
-- callable by anyone via the public Supabase REST API. An attacker
-- could call them directly with arbitrary p_user_id values, bypassing
-- all Next.js API route auth checks and placing trades on behalf of
-- any user.
--
-- Fixes:
-- 1. Add auth.uid() verification inside execute_trade and execute_sell
--    so that direct client calls must match the authenticated user.
-- 2. Add is_admin checks inside create_market and resolve_market.
-- 3. REVOKE EXECUTE from anon and public on all sensitive functions.
-- 4. GRANT EXECUTE only to authenticated and service_role.
-- ============================================

-- ============================================
-- 1. Secure execute_trade
-- ============================================
...
BEGIN
  ...
  -- When called via authenticated client (direct RPC), auth.uid() must match p_user_id.
  -- When called via service_role (server API route), auth.uid() is NULL — allowed
  -- because the API route already verified the user's session.
  IF auth.uid() IS NOT NULL AND auth.uid() != p_user_id THEN
    RAISE EXCEPTION 'Unauthorized: caller does not match user ID';
  END IF;
...
-- ============================================
-- 3. Secure create_market (admin-only)
-- ============================================
...
BEGIN
  ...
  IF auth.uid() IS NOT NULL THEN
    ...
    IF auth.uid() != p_creator_id THEN
      RAISE EXCEPTION 'Unauthorized: caller does not match creator ID';
    END IF;
  END IF;
...
-- ============================================
-- 4. Secure resolve_market (admin-only)
-- ============================================
...
-- ============================================
-- 6. REVOKE direct execution from anon/public,
--    GRANT only to authenticated and service_role
-- ============================================

-- execute_trade
REVOKE EXECUTE ON FUNCTION public.execute_trade(uuid, uuid, text, numeric) FROM anon;
REVOKE EXECUTE ON FUNCTION public.execute_trade(uuid, uuid, text, numeric) FROM public;
GRANT EXECUTE ON FUNCTION public.execute_trade(uuid, uuid, text, numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.execute_trade(uuid, uuid, text, numeric) TO service_role;

-- execute_sell
REVOKE EXECUTE ON FUNCTION public.execute_sell(uuid, uuid, text, numeric) FROM anon;
REVOKE EXECUTE ON FUNCTION public.execute_sell(uuid, uuid, text, numeric) FROM public;
GRANT EXECUTE ON FUNCTION public.execute_sell(uuid, uuid, text, numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.execute_sell(uuid, uuid, text, numeric) TO service_role;

-- create_market
REVOKE EXECUTE ON FUNCTION public.create_market(uuid, text, text, numeric, numeric) FROM anon;
REVOKE EXECUTE ON FUNCTION public.create_market(uuid, text, text, numeric, numeric) FROM public;
GRANT EXECUTE ON FUNCTION public.create_market(uuid, text, text, numeric, numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_market(uuid, text, text, numeric, numeric) TO service_role;
...
-- verify_qr_code
REVOKE EXECUTE ON FUNCTION public.verify_qr_code(uuid, text) FROM anon;
REVOKE EXECUTE ON FUNCTION public.verify_qr_code(uuid, text) FROM public;
GRANT EXECUTE ON FUNCTION public.verify_qr_code(uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.verify_qr_code(uuid, text) TO service_role;
